...
int cwmp_event_global_init(cwmp_t * cwmp)
{
    ...
    {
    	cwmp_log_error("event_filename:%s ,not exist!");
        return CWMP_ERROR;
    }
	...
	//here can read a event struct data from file, that saved after tags run over.
...
int cwmp_event_init(cwmp_t *cwmp)
{
	...
	cwmp_log_info("event init :%d, %s, %d\n", cwmp->event_global.event_flag, cwmp->event_global.event_key, cwmp->event_global.fault_code);
	
	//memset(cwmp->event_global.event_key ,0, sizeof(cwmp->event_global.event_key));
    if(cwmp->event_global.event_flag == EVENT_REBOOT_NONE_FLAG || cwmp->event_global.event_flag & EVENT_REBOOT_BOOTSTRAP_FLAG)
    {
    	printf("init event will? ");
        cwmp->event_global.event_flag = EVENT_REBOOT_NONE_FLAG;//EVENT_REBOOT_BOOTSTRAP_FLAG;
        cwmp_event_set_value(cwmp, INFORM_BOOTSTRAP, 1, NULL, 0, 0, 0);
        //cwmp_event_set_value(cwmp, INFORM_BOOT, 1, "", 0, 0, 0);
    }
    else    //reboot
    {
...
int cwmp_event_set_value(cwmp_t *cwmp,  int event,   int value, const char * cmd_key, int fault_code, time_t start, time_t end)
{
    ...
    ec->end = end;
    cwmp->el->count ++;
	printf("NO:%d set code:%s, key:%s fault-code:%d\n", k,ec->code, ec->command_key, ec->fault_code);
...
int cwmp_event_get_index(cwmp_t *cwmp,int event)
{
    int index = -1;
    int i;
    ...
    for(i=0; i<INFORM_MAX; i++)
    {
		...
		if(cwmp->el->events[i]->event == event)
        {
            index = i;
