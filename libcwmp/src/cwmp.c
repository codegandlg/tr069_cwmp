...
#include "cwmp_cwmp.h"
#include "cwmp_buffer.h"
#include "cwmp_log.h"
#include "cwmp_event.h"
#include "cwmp_private.h"


#define CWMP_BUF_SIZE 64
...
xmlnode_t * cwmp_xml_create_child_node(env_cwmp * env ,  xmlnode_t * parentNode, const char * ns, const char * nodeName, const char * nodeValue)
{
    XmlElement * newNode;
...
int cwmp_xml_set_node_attribute(env_cwmp * env,  xmlnode_t * node, const char * name, const char * value)
{
...
parameter_node_t * cwmp_initialize_parameter_node(env_cwmp * env ,
...
int  cwmp_add_parameter_to_list(env_cwmp * env ,  parameter_list_t * pl, parameter_t * parameter)
{
...
void  cwmp_add_event_to_list(env_cwmp * env ,  event_list_t * eventList, event_code_t * event)
{
...
int cwmp_get_parameter_node_value(cwmp_t * cwmp, parameter_node_t * node, const char * name, char ** value, pool_t * pool)
{
    ...
    if (!node)
    {
    	printf("here node is NULL\n");
        return FAULT_CODE_9000;
    }
    if (node->get)
    {
    	//printf("to do node->get fun, name:%s\n", name);
        return (*node->get)(cwmp, name, value, pool);
    }
    else
    {
    	printf("node->get fun is NULL \n");
        return FAULT_CODE_9000;
    }
}
...
int cwmp_parse_getparameternames_message(env_cwmp * env, xmldoc_t * doc, char ** path_name, unsigned int * next_level, fault_code_t *fault)
{
...
int cwmp_parse_getparametervalues_message(env_cwmp * env , xmldoc_t * doc, parameter_node_t * root, parameter_list_t ** ppl, fault_code_t *fault)
{
...
int cwmp_parse_getparameterattributes_message(env_cwmp * env , xmldoc_t * doc, parameter_node_t * root, parameter_list_t ** ppl, fault_code_t *fault)
{
	xmlnode_t * parameterListNode;
	xmlnode_t * parameterNode;
	parameter_t ** nextpv;
...
int  cwmp_parse_setparametervalues_message(env_cwmp * env , xmldoc_t * doc, parameter_node_t * root, parameter_list_t ** ppl, fault_code_t *fault)
{
    xmlnode_t * parameterListNode;
    xmlnode_t * parameterNode;
    parameter_t ** nextpv;
    ...
    *ppl = cwmp_create_parameter_list(env);
    ...
    while (parameterNode)
    {
        ...
        {
            //faild
            cwmp_log_error("faild here fun:%s:%d", __FUNCTION__, __LINE__);
            continue;
        }
        parameter_node_t * pn = cwmp_get_parameter_node(root, name);
        if (!pn)
        {
            ...
            cwmp_log_error("here can not find parameter %s.", name);
            rc = CWMP_ERROR;
            continue;
        }
        else
...
int cwmp_parse_download_message(env_cwmp * env , xmldoc_t *doc, download_arg_t ** pdlarg, fault_code_t *fault)
{
...
int cwmp_parse_upload_message(env_cwmp * env , xmldoc_t *doc, upload_arg_t ** pularg, fault_code_t *fault)
{
...
int cwmp_parse_addobject_message(env_cwmp * env , xmldoc_t *doc, parameter_node_t * root,  int * instances, int* status, fault_code_t *fault)
{
...
int cwmp_parse_deleteobject_message(env_cwmp * env , xmldoc_t *doc, parameter_node_t * root, int* status, fault_code_t *fault)
{
...
int cwmp_parse_reboot_message(env_cwmp * env , xmldoc_t *doc, char ** key, fault_code_t *fault)
{
    xmlnode_t * commandKeyNode;
...
parameter_t* cwmp_create_parameter(env_cwmp * env ,  const char * name, const char * value, size_t value_length, int type)
{
	cwmp_log_debug("name:%s,value:%s ,len:%d", name, value, value_length);
...
event_list_t * cwmp_create_event_list(env_cwmp * env, int size )
{
    event_list_t * el;
...
event_code_t * cwmp_create_event_code(env_cwmp * env )
{
    event_code_t * ev;
...
parameter_list_t* cwmp_create_parameter_list(env_cwmp * env )
{
    parameter_list_t * pl;
...
xmlnode_t * cwmp_create_current_time_node(env_cwmp * env ,   xmlnode_t * parent, const datatime_t *currentt)
{
...
xmlnode_t * cwmp_create_event_node(env_cwmp * env ,  xmlnode_t * parent, const event_list_t * eventlist)
{
    ...
    while (count < eventlist->count)
    {
		...
		cwmp_log_debug("event:%d commandkey:%s",pe[count]->event, pe[count]->command_key);
        ...
        if ( pe[count]->event == INFORM_BOOTSTRAP||pe[count]->event == INFORM_BOOT) //||
        {        	
            ESA(eventCommandKeyNode, cwmp_xml_create_child_node(env ,  eventStructNode, NULL, "CommandKey", NULL));
        }
        else if (pe[count]->event ==  INFORM_TRANSFERCOMPLETE)
        {
        	transfer = TRUE;
...
xmlnode_t * cwmp_create_header_node(env_cwmp * env ,   xmlnode_t * root, header_t * header)
{
...
xmlnode_t * cwmp_create_body_node(env_cwmp * env ,  xmlnode_t * root)
{
    xmlnode_t * bodyNode;
    FUNCTION_TRACE();
...
xmlnode_t * cwmp_create_envelope_node(env_cwmp * env ,  xmlnode_t * parent)
{
...
xmlnode_t * cwmp_create_device_id_node(env_cwmp * env ,  xmlnode_t * parent, const device_id_t * deviceid)
{
...
xmlnode_t * cwmp_create_max_envelope_node(env_cwmp * env ,  xmlnode_t * parent, unsigned int max_envelope)
{
...
xmlnode_t * cwmp_create_retry_count_node(env_cwmp * env ,  xmlnode_t * parent, unsigned int retry_count)
{
...
int cwmp_create_parameter_node(env_cwmp * env ,  parameter_node_t ** news, const char * name)
{
...
xmlnode_t * cwmp_create_parameter_list_node(env_cwmp * env ,  xmlnode_t * parent, parameter_list_t * pl)
{
...
xmldoc_t* cwmp_create_inform_message(env_cwmp * env ,  header_t * header,
                                     ...
                                     parameter_list_t * pl)
{
	...
	printf("--------fun:%s--%s------------\n", __FUNCTION__, deviceid->manufactorer);
...
xmldoc_t * cwmp_create_getrpcmethods_response_message(env_cwmp * env ,  header_t * header, char ** methods, unsigned int count)
{
    xmlnode_t * envelopeNode;
    xmlnode_t * bodyNode;
...
void * cwmp_create_getparameternames_response_all_parameter_names(env_cwmp * env , xmlnode_t * parent_node, const char * path_name, parameter_node_t *  param_node, int * count)
{
...
xmldoc_t* cwmp_create_getparameternames_response_message(env_cwmp * env ,
        header_t * header,
...
xmldoc_t* cwmp_create_getparametervalues_response_message(env_cwmp * env ,  header_t * header, parameter_list_t * pl)
{
    xmlnode_t * envelopeNode;
    xmlnode_t * bodyNode;
    xmlnode_t * responseNode;
...
xmldoc_t * cwmp_create_setparametervalues_response_message(env_cwmp * env ,  header_t * header, unsigned int status)
{
    xmlnode_t * envelopeNode;
    xmlnode_t * bodyNode;
    xmlnode_t * responseNode;
...
void cwmp_creat_datetimes_string(char * timebuf)
{
	struct tm t;
	time_t tn;
	datatime_t *now;
	...
	tn = time(NULL);
	t = *localtime(&tn);
	now = calloc(1,sizeof(datatime_t));
	...
	TRsnprintf(timebuf, 32, "%4d-%02d-%02dT%02d:%02d:%02d",
               now->year,
               now->month,
               now->day,
               now->hour,
    ...
    cwmp_log_debug("timebuf:%s", timebuf);	
}


xmldoc_t * cwmp_create_download_response_message(env_cwmp * env , header_t * header, int status)
{
    xmlnode_t * envelopeNode;
    xmlnode_t * bodyNode;
    xmlnode_t * responseNode;
	...
	cwmp_creat_datetimes_string(timebuf);
    bodyNode        = cwmp_create_body_node(env ,  envelopeNode);
    ESA(responseNode, cwmp_xml_create_child_node(env ,  bodyNode, NULL, CWMP_RPC_DOWNLOADRESPONSE, NULL));
    ESA(statusNode, cwmp_xml_create_child_node(env ,  responseNode, NULL, "Status", status == 0 ? "0" : "1"));
    ESA(startTimeNode, cwmp_xml_create_child_node(env ,  responseNode, NULL, "StartTime", timebuf));
    ESA(completeTimeNode, cwmp_xml_create_child_node(env ,  responseNode, NULL, "CompleteTime", timebuf));
    
    return doc;
}

xmldoc_t * cwmp_create_upload_response_message(env_cwmp * env , header_t * header, int status)
{
    xmlnode_t * envelopeNode;
    xmlnode_t * bodyNode;
    xmlnode_t * responseNode;
...
xmldoc_t * cwmp_create_addobject_response_message(env_cwmp * env , header_t * header, int instances, int status)
{
    xmlnode_t * envelopeNode;
    xmlnode_t * bodyNode;
    xmlnode_t * responseNode;
...
xmldoc_t * cwmp_create_deleteobject_response_message(env_cwmp * env , header_t * header, int status)
{
    xmlnode_t * envelopeNode;
    xmlnode_t * bodyNode;
    xmlnode_t * responseNode;
...
xmldoc_t * cwmp_create_reboot_response_message(env_cwmp * env ,  header_t * header)
{
    xmlnode_t * envelopeNode;
    xmlnode_t * bodyNode;
    xmlnode_t * responseNode;
...
xmldoc_t * cwmp_create_factoryreset_response_message(env_cwmp * env ,  header_t * header)
{
    xmlnode_t * envelopeNode;
    xmlnode_t * bodyNode;
    xmlnode_t * responseNode;
...
xmldoc_t* cwmp_create_faultcode_response_message(env_cwmp * env , header_t * header, fault_code_t * fault)
{
...
xmldoc_t* cwmp_create_faultcode_setparametervalues_response_message(env_cwmp * env , header_t * header, parameter_list_t * param_list, fault_code_t * fault)
{
...
char * cwmp_time_format(time_t gtime,char * buffer)
{
	struct tm * p;
	memset(buffer, 0, CWMP_BUF_SIZE);
	...
	cwmp_log_debug("time:%s\n", buffer);
	return buffer;
}

xmldoc_t * cwmp_create_transfercomplete_message(env_cwmp * env ,  header_t * header, cwmp_t * cwmp)
{
    xmlnode_t * envelopeNode;
    xmlnode_t * bodyNode;
    xmlnode_t * rpcNode;
	...
	int i = cwmp_event_get_index(cwmp,INFORM_TRANSFERCOMPLETE);
	evcode = cwmp->el->events[i];
	//printf("code:%s, key:%s fault:%d\n", evcode->code, evcode->command_key, evcode->fault_code);
    bodyNode     = cwmp_create_body_node(env ,  envelopeNode);
    
    ESA(rpcNode, cwmp_xml_create_child_node(env ,  bodyNode, NULL, CWMP_RPC_TRANSFERCOMPLETE, NULL));
    
    
    ESA(node, cwmp_xml_create_child_node(env ,  rpcNode, NULL, "CommandKey", (evcode->command_key)?(evcode->command_key):(NULL)));
	...
	char tm_buffer [CWMP_BUF_SIZE] = {0};	
	cwmp_time_format(evcode->start, tm_buffer);	
    ESA(node, cwmp_xml_create_child_node(env ,  rpcNode, NULL, "StartTime", tm_buffer)); 
    cwmp_time_format(evcode->end, tm_buffer);
    ESA(node, cwmp_xml_create_child_node(env ,  rpcNode, NULL, "CompleteTime", tm_buffer));
...
int cwmp_write_xmldoc_to_chunk(xmldoc_t *  doc, cwmp_chunk_t * chunk, pool_t * pool)
{
